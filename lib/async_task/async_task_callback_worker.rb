class AsyncTaskCallbackWorker
  ...
  # If job is called with qplum_perform_in, it tracks the job state in AsyncTask model
  # The last arguement is task specific which is separated here and rest is passed on the worker as arguments.
  # Instead of defining a reschedule method separately, use qplum_reschedule_after method which keeps track of retry counts in the model.
  
  extend QPLUM::AsyncJobScheduler
  ...
  def qplum_reschedule_after(perform_after, *args)
